...
RigidBody::RigidBody(double mass, double density, unsigned int type, double width, double height, sf::Texture& texture,
    ...
    //     //FROM WIKIPEDIA
    // }
    this->sprite_ = new SpriteObject(texture, sf::Vector2f(posX, posY));
    this->setScale({1, 1});
...
void RigidBody::setScale(sf::Vector2f scale)
{
    ...
    sf::FloatRect bounds = this->sprite_->sprite_.getLocalBounds();
...
void RigidBody::DisplayBodies(sf::RenderWindow &window, std::vector<RigidBody*> *rigid_bodies) {
    ...
    for (auto & rigid_bodie : *rigid_bodies) {
        if(rigid_bodie->visible) {
            rigid_bodie->sprite_->draw(window);
        }
    }
}
...
void RigidBody::ode(std::vector<RigidBody*> *y0, std::vector<RigidBody> *yEnd, double t0, double t1,
    ...
    for(int i = 0; i < y0->size(); i++) {
        ...
        y0->at(i)->sprite_->setPosition({yEnd->at(i).x.x, yEnd->at(i).x.y});
        y0->at(i)->sprite_->sprite_.rotate(yEnd->at(i).w);
        ...
        for (int j = 0; j < obstacles.size(); j++) {
            ...
            if (distance < (rigid_bodies->at(i)->radius + (obstacles.at(j)->getShape().getSize().y / 2))) {
                ...
                if (obstacles.at(j)->getCollisionPointDir() == 0) { // this is top border
                    ...
                    sf::Texture t = *rigid_bodies->at(i)->sprite_->sprite_.getTexture();
                    RigidBody *rigidBody_border = new RigidBody(100, 100, 1, obstacles.at(j)->getShape().getSize().x,
                                                                obstacles.at(j)->getShape().getSize().y, t, true,
